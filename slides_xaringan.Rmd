...
ggplot(cases, aes(x=V1)) + geom_histogram() + geom_vline(xintercept=0.5, linetype="dashed") +
  xlab("p") + ylab("P(p)") +
  theme_minimal() +
...
```{r, echo=F, eval=T, warning=F, message=F, fig.height=5, fig.align='center', fig.width=12}
cases <- read.csv("data/cancer_predictives.csv",header=F)
ggplot(cases, aes(x=V2)) + geom_histogram() + geom_vline(xintercept=0.5, linetype="dashed") +
  xlab("p") + ylab("P(p)") +
  theme_minimal() +
